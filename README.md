...
Phone-first Japanese trip practice. A guided five-minute session combines weak phrases, no-furigana readings, and one complete offline mission. Japanese-first cards train meaning, fixed replies, and readings; six deterministic missions rehearse complete exchanges and the abort. Every kanji has contextual ruby and an independently tested reading skill. A prerequisite DAG and fixed-parameter BKT choose the next skill; the SVG map explains and steers that choice. Optional HTTP roleplay is a reviewed sensor, never the mastery authority.
...
Progress stays in this browser's `localStorage` at `kaiwa.practice-state.v1`. State schema v4 stores per-skill BKT, card/mission/roleplay/hint evidence, cram timing, map focus, resumable missions and guided sessions, and local completion/response-time metrics. Old state migrates automatically. The footer can download or restore a JSON progress backup without a network. A phrase is ready only after its BKT threshold **and two correct observations**, preventing one lucky choice from unlocking the DAG. New or weak words show furigana; reading cards and mission challenge runs hide it. Ruby retires only after 75% reading BKT **and two consecutive no-furigana passes**. Phrase and mission successes never inflate the readings inside the line.
...
Core paths: `data/`, `src/readings.js`, `src/mastery.js`, `src/scheduler.js`, `src/session.js`, `src/mission.js`, `src/map.js`, `src/store.js`, `src/ui.js`, and `server/roleplay.js`.
