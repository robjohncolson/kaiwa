...
_Post-slice note:_ follow-up work added the constrained optional P3 proxy, bounded guided sessions, speak-first evidence, and a field-repair loop. A difficult real result now selects one prompt, the weakest contextual reading, the weakest fixed response, the timed abort, and a persisted ten-minute spoken revisit. The repair never rewrites the field result; production can steer practice but cannot inflate BKT or unlock the DAG. Reading BKT still needs consecutive unaided passes before furigana retires. The recommendation to keep routed roleplay out of the core validation loop stands.
