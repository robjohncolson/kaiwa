...
_Post-slice note:_ follow-up work added the constrained optional P3 proxy, bounded guided sessions, universal speak-first evidence, Showtime, and a field-repair loop. The UI is now a sitewide two-action wizard: three candidates appear sequentially as no/yes decisions, and field results use a short binary tree. All fixed learner lines can be recalled aloud before reveal, while guided practice balances recognition, production, word facets, and a mission. A difficult result still selects one prompt, the weakest contextual reading, the weakest fixed response, the timed abort, and a persisted ten-minute revisit. Production cannot inflate BKT or rewrite the field log; a scheduled event becomes its phone sheet and then its field-log handoff instead of silently expiring. The recommendation to keep routed roleplay out of the core validation loop stands.
