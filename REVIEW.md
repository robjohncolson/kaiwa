...
_Post-slice note:_ follow-up work added the constrained optional P3 proxy, bounded guided sessions, speak-first evidence, and a field-repair loop. The UI is now a sitewide two-action wizard: three candidates appear sequentially as no/yes decisions, and field results use a short binary tree. This reduces visible choice without weakening the underlying evidence boundary. A difficult result still selects one prompt, the weakest contextual reading, the weakest fixed response, the timed abort, and a persisted ten-minute revisit. Production cannot inflate BKT or rewrite the field log. The recommendation to keep routed roleplay out of the core validation loop stands.
