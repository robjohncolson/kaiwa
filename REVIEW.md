...
_Post-slice note:_ follow-up work added the constrained optional P3 proxy and a bounded offline session: three recognition cards, three independent reading checks, and one speak-first mission. Reading BKT alone no longer retires furigana; it also requires consecutive unaided passes. Production self-grades and real-world outcomes are now stored separately: they can steer practice but cannot inflate BKT or unlock the DAG. The recommendation to keep routed roleplay out of the core validation loop still stands.
